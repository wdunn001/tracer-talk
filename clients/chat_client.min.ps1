...
tracert -h 1 -w 1000 "$(E $M).tx.$Z" 2>&1|Out-Null;$R="";tracert -w 2000 "rx.$Z" 2>&1|?{$_-match"\.${Z}"-and$_-notmatch"(end|empty|rx|Tracing)"}|%{if($_-match"(\S+)\.$Z"){$R+=(($Matches[1]-split'\.'|?{$_.Length%2 -eq 0 -and $_ -match '^[0-9a-f]+$'})-join'')}};if($R){Write-Host "< $(D $R)"};Start-Sleep {{POLL_RATE}}}
