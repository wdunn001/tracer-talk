...
tracert -h 1 -w 1000 "$(E $M).tx.$Z" 2>&1|Out-Null;$R="";tracert -w 2000 "rx.$Z" 2>&1|?{$_-match"\.${Z}"-and$_-notmatch"(end|empty|rx|Tracing)"}|%{if($_-match"(\S+)\.$Z"){$R+=($Matches[1]-replace"\.","")}};if($R){Write-Host "< $(D $R)"}}
